...
- `build` (`.github/workflows/release.yml:11-70`) — matrix over `linux-x86_64`, `macos-aarch64`,
  `macos-x86_64` (`.github/workflows/release.yml:20-30`). Each leg builds with
  `cargo build --release --locked`, tars the binary, and checksums it with `shasum -a 256`.
- `build-windows` (`.github/workflows/release.yml:72-137`) — single job on `windows-latest`.
...
- `notes` (`.github/workflows/release.yml:139-154`) — `needs: [build, build-windows]`
  (`.github/workflows/release.yml:141`), so release notes are only generated after every
...
doesn't match `RELEASE_TAG` (`.github/workflows/release.yml:94-101`, mirrored in `build` at
`.github/workflows/release.yml:46-52`) — this is what forces `Cargo.toml`'s version to be bumped
...
binary (`.github/workflows/release.yml:103-104`):
...
(`.github/workflows/release.yml:120`) so `cargo-wix` packages that binary instead of rebuilding.
...
(`.github/workflows/release.yml:60-61`, in `build`) and the Windows zip checksum
(`.github/workflows/release.yml:111-114`) satisfy this naturally since they run with the artifact
in the current directory. The MSI is built into `target/wix/`, so its checksum step `cd`s there
first — `(cd target/wix && sha256sum "...")` (`.github/workflows/release.yml:122-125`) — rather
